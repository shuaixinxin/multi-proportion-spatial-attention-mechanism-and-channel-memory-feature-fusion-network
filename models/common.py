...
class ReOrg(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)   通道数扩大四倍  特征图尺寸下采样四倍
...
class Chuncat(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = []
        for xi in x:
            xi1, xi2 = xi.chunk(2, self.d)  #对d 维度 进行拆分 拆分成两份
            x1.append(xi1)
            x2.append(xi2)
...
def DWConv(c1, c2, k=1, s=1, act=True):
    ...
    return Conv(c1, c2, k, s, g=math.gcd(c1, c2), act=act)     # math.gcd(a,b) 最大公约数
...
class ReOrg(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
        return torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]], 1)
class ROUpsample(nn.Module):
    def __init__(self, c1, c2):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.cv1 = Conv(int(c1*4), c2)
        self.reorg = ReOrg()
        self.xx = nn.Upsample(None, 2, 'nearest')

    def forward(self, x):
        x1 = self.reorg(x)# ReOrg: (b, c, w, h) => (b, 4c, w/2, h/2)
        x2 = self.cv1(x1)
        x3 = self.xx(x)
        return x2